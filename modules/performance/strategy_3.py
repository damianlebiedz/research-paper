...
def generate_trade(position: float, prev_position: float, q_x: float, q_y: float, w_x: float, w_y: float,
                   ...
                   alpha_pos: float, z_score: float, spread: float, price_x: float, price_y: float, mean: float,
                   std: float, initial_cash: float, fee_rate: float, total_fees: float,
    ...
    if position != 0:
        ...
        if beta < 0:
            return 0, None, None, 0, total_fees, None, None, 0, 0, None, None, 0, None
        elif position < 0:
            if z_score_virt <= exit_threshold or (
                ...
                return 0, beta_pos, alpha_pos, pnl, total_fees, z_score_virt, spread_virt, 0, 0, None, None, 0, None
        else:
            if z_score_virt >= -exit_threshold or (
                    stop_loss_threshold is not None and z_score_virt <= -stop_loss_threshold):
                # CLOSE POSITION
        ...
        if prev_position == position:
            ...
            return prev_position, beta_pos, alpha_pos, pnl, total_fees, z_score_virt, spread_virt, q_x, q_y, w_x, w_y, entry_val, stop_loss_threshold
        else:
            ...
            return position, beta_pos, alpha_pos, pnl, total_fees, z_score, spread, q_x, q_y, w_x, w_y, entry_val, stop_loss_threshold
    else:
        # OUT OF POSITION
        if position != 0:
            ...
            return position, beta_pos, alpha_pos, 0, total_fees, z_score, spread, q_x, q_y, w_x, w_y, entry_val, stop_loss_threshold
        else:
...
def run_strategy(pair_data: Pair, entry_threshold: float, exit_threshold: float, stop_loss: float,
    ...
    for i in range(len(df)):
        if total_pnl == -initial_cash:
            ...
            prev_pos, beta_pos, alpha_pos, pnl, total_fees, z_score_virt, spread_virt, q_x, q_y, w_x, w_y, entry_val, stop_loss_threshold = generate_trade(
                position, prev_pos, q_x, q_y, w_x, w_y, exit_threshold, stop_loss, beta, beta_pos, alpha_pos, z_score,
                spread, price_x, price_y, mean, std, initial_cash, fee_rate, total_fees, entry_val,
